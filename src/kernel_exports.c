/* Xbox kernel export ordinal -> name (xboxkrnl.exe, retail). 371 entries.
 * Ported from the C# branch (src/.../XboxKernelExports.cs). */
#include "kernel_exports.h"
#include <stddef.h>
#include <stdio.h>

typedef struct { int ordinal; const char* name; } KExport;

static const KExport k_exports[] = {
...
const char* xbox_kernel_export_name(int ordinal) {
    for (size_t i = 0; i < sizeof(k_exports) / sizeof(k_exports[0]); i++)
        if (k_exports[i].ordinal == ordinal) return k_exports[i].name;
    return NULL;
}

int xbox_kernel_export_name_buf(int ordinal, char* buf, size_t n) {
    const char* s = xbox_kernel_export_name(ordinal);
    if (s) { snprintf(buf, n, "%s", s); return 1; }
    snprintf(buf, n, "Ordinal_%d", ordinal);
    return 0;
}

...
static void kernel_thunk_dispatch(void)
{
    ...
    if (slot < 0 || slot >= XBOX_KERNEL_THUNK_TABLE_SIZE) {
        ...
        c->eax = 0;
        c->esp += 4;  /* pop dummy return address */
        return;
    }
     ...
     * reset c->esi when it looked out of range, overwrote esi+0x104 and
    ...
    if (g_kernel_call_count <= 4000) {
        fprintf(stderr, "  [KERNEL] #%d: ordinal %u (slot %d) esp=0x%08X\n",
                g_kernel_call_count, ordinal, slot, c->esp);
        fflush(stderr);
    }
